...
static OBJ TrVM_defclass(VM, OBJ name, TrBlock *b, int module, OBJ super) {
  ...
  TR_WITH_FRAME(f, mod, mod, 0, {
    TrVM_interpret(vm, f, b, 0, 0, 0, 0);
  });
  return mod;
}
...
static inline OBJ TrVM_yield(VM, TrFrame *f, int argc, OBJ argv[]) {
  ...
  OBJ ret = TR_NIL;
  TR_WITH_FRAME(f, cl->self, cl->class, cl->parent, {
    ret = TrVM_interpret(vm, f, cl->block, 0, argc, argv, cl);
  });
  return ret;
}
...
static OBJ TrVM_interpret(VM, register TrFrame *f, TrBlock *b, int start, int argc, OBJ argv[], TrClosure *closure) {
  ...
  OPCODES;
    ...
    /* return */
    OP(RETURN):     /* TODO for GC: release everything on the stack before returning */ return R[A];
