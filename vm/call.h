...
#define TR_WITH_FRAME(FRM,SELF,CLASS,CLOS,BODY) ({ \
  /* push a frame */ \
  if (unlikely(++vm->cf >= TR_MAX_FRAMES)) tr_raise(SystemStackError, "Stack overflow"); \
  TrFrame _FRM; \
  register TrFrame *FRM = &_FRM; \
  FRM->previous = vm->frame; \
  if (vm->cf == 0) vm->top_frame = FRM; \
  vm->frame = FRM; \
  FRM->self = SELF; \
  FRM->class = CLASS; \
  FRM->closure = CLOS; \
...
static inline OBJ TrMethod_call(VM, OBJ self, OBJ receiver, int argc, OBJ *args, int splat, TrClosure *cl) {
  OBJ ret = TR_NIL;
  TR_WITH_FRAME(f, receiver, TR_CLASS(receiver), cl, {
